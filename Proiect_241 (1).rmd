...
subtitle: "Probabilitati si Statistica Grupa 241"
author: "Bara Tudor, Marcu Florian, Negulescu Radu, Pavalasc Irina"
date: "2/4/2021"
...
"ui" reprezinta interfata utilizatorului, in acesta regasindu-se un panel care cuprinde cate un tabPanel pentru fiecare repartitie in parte. Totodata, aici se gasesc si functiile de input si output specifice aplicatiei Shiny.
...
In plus, pentru reprezentarea probabilitatilor pentru repartitiile continue am folosit functiile ajutatoare polygon care hasureaza o parte din grafic si plot_polygon care apeleaza pentru diferite intervale
...
server <- function(input, output, session) {
  ...
  output$masa_exponentiala <- renderPlot({
    ...
    dat <- data.frame(x=x, px=dexp(x, rate=input$rata))
    ggplot(dat, aes(x=x, y=px)) + geom_line() + ggtitle("Densitatea")
  })
  ...
  output$densitatea_beta <- renderPlot({
    x <- seq(0, 1,by = 0.02)
    plot(x,dbeta(x, input$shape1_beta, input$shape2_beta), type = "b", xlab = "k", main = "Densitatea repartitiei beta")
  })
  ...
  output$densitatea_gamma <- renderPlot({
    x <- seq(0, 20)
    plot(x,dgamma(x, input$shape_gamma, input$rata_gamma), type = "b", xlab = "k", main = "Densitatea repartitiei gamma")
  })
  ...
  output$chi_dens <- renderPlot({
    ...
    plot(x, dchisq(x, df=input$chi_df, ncp=input$chi_cent), type="l", xlab = "X", ylab = "Chi", main = "Densitate Chi square")
  })
  output$chi_rep <- renderPlot({
    x <- seq(0, input$chi_nr, by=0.01)
    plot(x, pchisq(x, df=input$chi_df, ncp=input$chi_cent), 
         type="l", xlab = "X", ylab = "Chi", main = "Functia de repartitie Chi-Squared && P(x<=a) P(a<=x<=b) P(x>=b)")
  ...
  output$stud_dens <- renderPlot({
    ...
    plot(function(x) dt(x, df=input$stud_df, ncp=input$stud_cent), input$stud_nr / -2, input$stud_nr,
         xlab = "X", ylab = "t", main = "Densitatea repartitiei student")
  })
  output$stud_rep <- renderPlot({
    plot(function(x) pt(x, df=input$stud_df, ncp=input$stud_cent), input$stud_nr / -2, input$stud_nr,
         xlab = "X", ylab = "t", main = "Functia de repartitie student && P(x<=a) P(a<=x<=b) P(x>=b)")
    x <- seq(input$stud_nr / -2, input$stud_nr, by=0.1)
  ...
  output$lognorm_dens <- renderPlot({
    ...
    plot_continua(x, fun, xlab = "X", ylab = "Log Norm(mean, var)", main = "Densitate repartitiei log normala")
  })
  output$lognorm_rep <- renderPlot({
    x <- seq(0.001, input$lognorm_nr, by=0.01)
    fun <- plnorm(x, mean = input$lognorm_medie, sd = input$lognorm_var)
    plot_continua(x, fun, xlab = "X", ylab = "Norm(mean, var)", main = "Functia de repartitie log normala && P(x<=a) P(a<=x<=b) P(x>=b)")
  ...
  output$norm_dens <- renderPlot({
    ...
    plot(x, dnorm(x, mean = input$norm_medie, sd = input$norm_var), type = "l", lty=2, lwd=2,
         xlab = "X", ylab = "Norm(mean, var)", main = "Densitate repartitiei normale")
  })
  output$norm_rep <- renderPlot({
    x <- seq(-input$norm_nr, input$norm_nr)
    fun <- pnorm(x, mean = input$norm_medie, sd = input$norm_var)
    plot(x, fun, type = "l", lty=2, lwd=2, xlab = "X", ylab = "Norm(mean, var)", main = "Functia de repartitie normala && P(x<=a) P(a<=x<=b) P(x>=b)")
    plot_poligon(x = x,
                 a = input$norm_a,
                 b = input$norm_b,
                 fun = fun)
...
1. https://vimeo.com/rstudioinc/review/131218530/212d8a5a7a/#comment_14123410

2. https://sites.harding.edu/fmccown/r/

3. https://www.dummies.com/programming/r/how-to-create-different-plot-types-in-r/

4. https://stat.ethz.ch/

5. https://bookdown.org/yihui/rmarkdown/

6. https://www.statmethods.net/graphs/bar.html

7. https://www.rdocumentation.org
